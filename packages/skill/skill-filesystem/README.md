...
The two user roots (`user-dsh`, `user-agents`) are the same for every project on the machine: anything discovered there is, by default, offered in every session regardless of what that project actually is. A skill whose frontmatter declares `projects` narrows that: it is included from a user root only when the current project directory's basename appears in the list, and is otherwise skipped, as if it were never discovered. A skill with no `projects` field is unaffected, included everywhere exactly as before this field existed; this is why a skill that has drifted onto a user root without a `projects` field (a Cloudflare-specific skill installed under `~/.agents/skills`, say) surfaces its full catalog description in every unrelated project. `projects` is only consulted for the two user roots; a skill discovered from a project root, a custom root, or the bundled root ignores it, since those are already scoped by where they were placed or explicitly configured. With no `cwd` (a cold transcript read has none), no project resolves, scoping is unenforceable, and every skill is included, the same fallback an absent `projects` field gets.
...
`projects` is a non-empty array of non-empty strings, each a project directory's basename (`nexus-harness`, not an absolute path — the value has to travel to other machines' home directories, so it names the project rather than locating it). It only has an effect from the two user roots, described under Discovery above. A wrong-shaped value (not an array, an empty array, a non-string entry) warns and is treated as absent, unscoped, rather than dropping the whole skill: this field is advisory filtering, not the invocation-surface decision `disable-model-invocation` is, so failing open here does not risk exposing a skill somewhere it was meant to be hidden, at worst it shows up somewhere irrelevant, exactly the pre-existing behavior.
...
- **`projects` is opt-in per skill, not a project-relevance judgment** — nothing infers whether a skill actually suits a project's stack; an unscoped user-root skill still shows everywhere until its own frontmatter says otherwise. It also only covers the two user roots; a custom root configured through `customSkillDirs` is unfiltered by design for now, since that root is already an explicit, deliberate opt-in rather than an always-on default.
