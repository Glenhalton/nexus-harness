...
**添加 Ollama** 是同一套手工声明机制之上的一个成形的快捷方式，而非新的提供方门类：Ollama 的本地服务与 Ollama Cloud 都已经说通用卡片提供的 `openai-completions` 协议，因此这张卡片提交的是相同的 profile 形态，只是预填了 Ollama 的默认值，并把协议固定住而非交给用户选择。本地模式完全不渲染 API 密钥字段——提交的 profile 也从不设置 `apiKeyEnv`——并在打开时通过同一个 `llm.discoverModels` 询问自动探测 `http://localhost:11434/v1`，无需用户点击即可填好模型行；探测不到本地服务时，卡片旁会留下一行提示，且手动的**获取可用模型**重试按钮仍然可用。提交的 profile 仍会带上一个占位用的 `Authorization: Bearer ollama` 请求头：pi-ai 的 `openai-completions` 客户端在既没有 `apiKey` 也没有 `Authorization` 请求头时会拒绝构建聊天请求，无论目标端点是否真的校验它，而 Ollama 本地服务两者都不校验。云端模式把端点默认为 `https://ollama.com/v1`，并且在填入密钥之前不会启用创建，因为 Ollama Cloud 没有免鉴权的路径。在本地与云端之间切换时，只会替换仍持有上一模式默认值的字段，因此用户已编辑过的 Provider ID、显示名称或 API 地址会在切换后保留。
